...
const (
	...
	POST_TYPE = "post"
	DISTANCE = "200km"
	BUCKET_NAME = "img_liujz"
	// Needs to update this URL if you deploy it to cloud.
    ES_URL = "http://34.73.1.220:9200"
...
func handlerPost(w http.ResponseWriter, r *http.Request) {
    ...
    if err != nil {
        http.Error(w, "Failed to save image to GCS hahahahaha", http.StatusInternalServerError)
...
func handlerCluster(w http.ResponseWriter, r *http.Request) {
	...
	q := elastic.NewRangeQuery("face").Gte(0.9)
    fmt.Println(term)
    fmt.Println(q)
...
func saveToGCS(r io.Reader, bucketName, objectName string) (*storage.ObjectAttrs, error) {
    ...
    // Creates a client.
    client, err := storage.NewClient(ctx, option.WithCredentialsFile("My First Project-26035556b63a.json"))
    if err != nil {
        return nil, err
    }
...
func saveToBigTable(p *Post, id string) {
	ctx := context.Background()
	bt_client, err := bigtable.NewClient(ctx, "causal-scarab-226900", "around-post", option.WithCredentialsFile("My First Project-26035556b63a.json"))
	if err != nil {
		panic(err)
		return
	}
